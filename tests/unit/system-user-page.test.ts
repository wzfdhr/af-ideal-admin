...
import UserSystemPage from '@/views/system/userSystem/index.vue'
...
vi.mock('@/api/system/user', () => ({
  ...
  fetchSystemUsers: apiMocks.fetchSystemUsers,
  getSystemUserDetail: apiMocks.getSystemUserDetail,
  createSystemUser: apiMocks.createSystemUser,
  updateSystemUser: apiMocks.updateSystemUser,
  deleteSystemUser: apiMocks.deleteSystemUser,
}))
...
describe('UserSystemPage', () => {
  beforeEach(() => {
    ...
    apiMocks.fetchSystemUsers.mockResolvedValue({
      list: [
        {
          id: '1',
          username: 'admin',
    ...
    apiMocks.getSystemUserDetail.mockResolvedValue({
      id: '1',
      username: 'admin',
      name: '系统管理员',
      phone: '17666666666',
    ...
    apiMocks.deleteSystemUser.mockResolvedValue(undefined)
  })

  it('loads users through ProTable fetch contract', async () => {
    mountPage()
    await settle()

    expect(apiMocks.fetchSystemUsers).toHaveBeenCalledWith({
      current: 1,
      pageSize: 10,
    })
    expect(dictionaryMocks.getOptions).toHaveBeenCalledWith('userStatus')
  })
  ...
  it('submits query values to table reset filters', async () => {
    ...
    expect(apiMocks.fetchSystemUsers).toHaveBeenLastCalledWith({
      current: 1,
      pageSize: 10,
      username: 'admin',
      phone: '176',
  ...
  it('renders permission actions and confirms delete before calling api', async () => {
    ...
    expect(
      wrapper.find('[data-permission="system:user:create"]').exists()
    ).toBe(true)

    await wrapper
      .find('[data-permission="system:user:delete"]')
      .trigger('click')
    expect(apiMocks.deleteSystemUser).not.toHaveBeenCalled()

    await wrapper.find('[data-testid="user-delete-modal-ok"]').trigger('click')
    await settle()

    expect(apiMocks.deleteSystemUser).toHaveBeenCalledWith('1')
