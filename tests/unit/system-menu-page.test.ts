...
import MenuSystemPage from '@/views/system/menuSystem/index.vue'
import type { ProTableFetchParams } from '@/components/pro-table/types'
import type { PropType } from 'vue'

const apiMocks = vi.hoisted(() => ({
  fetchSystemMenus: vi.fn(),
  getSystemMenuDetail: vi.fn(),
  createSystemMenu: vi.fn(),
  updateSystemMenu: vi.fn(),
  deleteSystemMenu: vi.fn(),
}))
...
vi.mock('@/api/system/menu', () => ({
  ...
  fetchSystemMenus: apiMocks.fetchSystemMenus,
  getSystemMenuDetail: apiMocks.getSystemMenuDetail,
  createSystemMenu: apiMocks.createSystemMenu,
  updateSystemMenu: apiMocks.updateSystemMenu,
  deleteSystemMenu: apiMocks.deleteSystemMenu,
}))
...
describe('MenuSystemPage', () => {
  beforeEach(() => {
    ...
    apiMocks.fetchSystemMenus.mockResolvedValue({
      list: [
        {
          id: '1',
          menuName: '系统管理',
    ...
    apiMocks.getSystemMenuDetail.mockResolvedValue({
      id: '1',
      menuName: '系统管理',
      menuType: 'catalog',
      path: '/system',
    ...
    apiMocks.deleteSystemMenu.mockResolvedValue(undefined)
  })

  it('loads menus through ProTable fetch contract', async () => {
    mountPage()
    await settle()

    expect(apiMocks.fetchSystemMenus).toHaveBeenCalledWith({
      current: 1,
      pageSize: 10,
    })
    expect(dictionaryMocks.getOptions).toHaveBeenCalledWith('menuStatus')
  })
  ...
  it('submits query values to table reset filters', async () => {
    ...
    expect(apiMocks.fetchSystemMenus).toHaveBeenLastCalledWith({
      current: 1,
      pageSize: 10,
      menuName: '系统',
      status: 'enabled',
  ...
  it('renders permission actions and confirms delete before calling api', async () => {
    ...
    expect(
      wrapper.find('[data-permission="system:menu:create"]').exists()
    ).toBe(true)

    await wrapper
      .find('[data-permission="system:menu:delete"]')
      .trigger('click')
    expect(apiMocks.deleteSystemMenu).not.toHaveBeenCalled()

    await wrapper.find('[data-testid="menu-delete-modal-ok"]').trigger('click')
    await settle()

    expect(apiMocks.deleteSystemMenu).toHaveBeenCalledWith('1')
