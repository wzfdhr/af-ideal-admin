...
vi.mock('@/api/system/role', () => ({
  ...
  fetchSystemRoles: apiMocks.fetchSystemRoles,
  getSystemRoleDetail: apiMocks.getSystemRoleDetail,
  createSystemRole: apiMocks.createSystemRole,
  updateSystemRole: apiMocks.updateSystemRole,
  deleteSystemRole: apiMocks.deleteSystemRole,
}))
...
describe('RoleSystemPage', () => {
  beforeEach(() => {
    ...
    apiMocks.fetchSystemRoles.mockResolvedValue({
      list: [
        {
          id: '1',
          roleName: '超级管理员',
    ...
    apiMocks.getSystemRoleDetail.mockResolvedValue({
      id: '1',
      roleName: '超级管理员',
      roleKey: 'admin',
      roleSort: 1,
    ...
    apiMocks.deleteSystemRole.mockResolvedValue(undefined)
  })

  it('loads roles through ProTable fetch contract', async () => {
    mountPage()
    await settle()

    expect(apiMocks.fetchSystemRoles).toHaveBeenCalledWith({
      current: 1,
      pageSize: 10,
    })
    expect(dictionaryMocks.getOptions).toHaveBeenCalledWith('roleStatus')
  })
  ...
  it('submits query values to table reset filters', async () => {
    ...
    expect(apiMocks.fetchSystemRoles).toHaveBeenLastCalledWith({
      current: 1,
      pageSize: 10,
      roleName: '管理员',
      roleKey: 'admin',
  ...
  it('renders permission actions and confirms delete before calling api', async () => {
    ...
    expect(
      wrapper.find('[data-permission="system:role:create"]').exists()
    ).toBe(true)

    await wrapper
      .find('[data-permission="system:role:delete"]')
      .trigger('click')
    expect(apiMocks.deleteSystemRole).not.toHaveBeenCalled()

    await wrapper.find('[data-testid="role-delete-modal-ok"]').trigger('click')
    await settle()

    expect(apiMocks.deleteSystemRole).toHaveBeenCalledWith('1')
